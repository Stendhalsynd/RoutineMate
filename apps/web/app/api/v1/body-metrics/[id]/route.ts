...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    ...
    if (!parsed.success) {
      return badRequest("Invalid body metric delete payload.", zodIssues(parsed.error));
    }
    ...
    const deleted = await repo.softDeleteBodyMetric(session.userId, parsed.data.id);
    if (!deleted) {
    ...
    const message = error instanceof Error ? error.message : "Failed to delete body metric entry.";
