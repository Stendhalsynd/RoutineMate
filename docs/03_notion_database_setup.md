...
- `EXPO_PUBLIC_GOOGLE_ANDROID_CLIENT_ID=...` (선택: 서버/디버깅 보조값)
- `EXPO_PUBLIC_GOOGLE_ANDROID_REDIRECT_URI=...` (Deprecated: native SDK 전환 후 기본 미사용)
...
기본 정책:
- 모바일은 브라우저 OAuth가 아닌 `Google Sign-In native SDK` 흐름을 사용합니다.
- 로그인 전용이면 Google 데이터 액세스 범위는 `openid`, `email`, `profile`만 유지합니다.

1. Android 클라이언트 자격증명 점검
- `EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID`는 **Web 타입 클라이언트 ID**여야 하며, 모바일 SDK configure 값으로 사용됩니다.
- `GOOGLE_ANDROID_CLIENT_ID`는 서버의 `azp` 검증에 사용되므로 **Android 타입 클라이언트 ID**여야 합니다.
...
4. 네이티브 SDK 기준 리다이렉트
- Android native SDK 경로에서는 브라우저 기반 `redirect_uri` 수동 관리가 핵심 요인이 아닙니다.
- Console에서 Android OAuth Client(`package + SHA-1`)가 정확하면 됩니다.
- 기존 `EXPO_PUBLIC_GOOGLE_ANDROID_REDIRECT_URI`는 하위호환 변수이며 기본 경로에서는 사용하지 않습니다.
