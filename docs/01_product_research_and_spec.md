...
## 14) S5-5 운영 안정화 메모 (2026-03-06)

### 차트 경계 정책
- 모바일 체성분 차트는 viewport 추정치가 아닌 "실제 카드 컨테이너 폭"을 기준으로 렌더링한다.
- 웹/모바일 모두 차트 래퍼에 클리핑(`overflow: hidden`)을 적용해 카드 경계 밖 렌더를 차단한다.

### 배포 아티팩트 정책
- APK 릴리즈 업로드는 `release` 산출물만 허용한다.
- `preview/debug` APK는 릴리즈 업로드 단계에서 차단한다.
- 스크립트 검증은 dry-run 모드를 통해 CI/로컬에서 네트워크 없이 재현 가능해야 한다.
...
## 14) S4-8 UX/동기화 업데이트 (2026-03-03)

### 템플릿 모델
- 식단 템플릿은 슬롯(`MealSlot`) 구분을 사용하지 않고 전 슬롯 공통 목록으로 사용.
- 식단/운동 모두 활성 템플릿이 1개 이상 있어야 `함` 액션을 허용.

### 입력 UX
- 모바일: 기록 날짜/체성분 날짜, 리마인더 시간, 목표/체성분 수치 입력을 선택형 바텀시트로 전환.
- 웹: 날짜 필드는 필드 전체 클릭 가능한 `DateField`로 전환, 목표/체성분 수치는 선택형(`select`) 중심으로 전환.

### 대시보드 체감 개선
- 모바일 대시보드는 KPI 텍스트만 노출하지 않고 `미니 트렌드바`를 추가.
- 기록 mutation 이후 대시보드 재조회는 `fresh=1`을 사용해 캐시 지연을 최소화.
